...
//        }
        ...
        public async Task<CalendarEvent> getCalendarEventWithID(string ID, ReferenceNow now)
        {
            ...
            Dictionary<string, CalendarEvent> AllScheduleData = await getAllEnabledCalendar(RangeOfLookup, now);
        ...
        public async Task<IList<CalendarEvent>> getCalendarEventWithName(string Name)
        {
            ...
            TimeLine RangeOfLookup = new TimeLine(DateTimeOffset.UtcNow.AddYears(-1000), DateTimeOffset.UtcNow.AddYears(1000));
            Dictionary<string, CalendarEvent> AllScheduleData = await getAllEnabledCalendar(RangeOfLookup, null);// null for now object because for a name look up you don't need a now update
