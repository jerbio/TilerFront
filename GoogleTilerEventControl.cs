...
namespace TilerFront
{
    public class GoogleTilerEventControl
    {
        ...
        async public Task<GoogleThirdPartyControl> getThirdPartyControlForIndex()
        {
            ...
            List<CalendarEvent> RetValueList = (await GoogleCalExtension.getAllCalEvents(list.Items, CalendarServiceInfo, EmailID, googleAuthenticationID, null).ConfigureAwait(false)).ToList();
        ...
        /// </summary>
        /// <returns></returns>
        async static public Task<Tuple<List<GoogleTilerEventControl>, GoogleThirdPartyControl>> getThirdPartyControlForIndex(IEnumerable<GoogleTilerEventControl> AllGoogleControls, TimeLine calucaltionTimeLine)
        {
            ...
            for (int j = 0; j < AllControls.Count; j++)
            {
                ...
                AllConcurrentTasks.Add(eachGoogleTilerEventControl.getCalendarEvents(calucaltionTimeLine));
            }
        ...
        async public Task<List<CalendarEvent>> getCalendarEventsForIndex(TimeLine CalculationTimeLine)
        {
            ...
            EventID googleAuthenticationID = EventID.generateGoogleAuthenticationID(((TilerFront.Models.IndexedThirdPartyAuthentication)AuthenticationInfo).ThirdPartyIndex);
            RetValue = (await GoogleCalExtension.getAllCalEvents(list.Items, CalendarServiceInfo, EmailID, googleAuthenticationID, CalculationTimeLine).ConfigureAwait(false)).ToList();
            
            return RetValue;
        }

        async public Task<List<CalendarEvent>> getCalendarEvents(TimeLine calculationTimeLine)
        {
            //var PrepList = CalendarServiceInfo.Events.List(EmailID);
        ...
        static async public Task<ConcurrentBag<CalendarEvent>> getAllCalEvents(IEnumerable<GoogleTilerEventControl>AllGoogleCalControl, TimeLine CalculationTimeLine)
        {
            ...
            {
                ConcurrentTask.Add(obj.getCalendarEventsForIndex(CalculationTimeLine));
            }
            //);
